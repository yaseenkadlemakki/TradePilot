...
# 1
async def test_output_contains_exactly_4_proposals(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    assert len(output.proposals) == 4


# 2
async def test_all_four_strategy_types_present(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
...
# 3
async def test_each_proposal_has_ticker(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
...
# 4
async def test_each_proposal_has_strike(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    for p in output.proposals:
        assert p["strike"] > 0
...
# 5
async def test_each_proposal_has_expiry(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    for p in output.proposals:
...
# 6
async def test_long_call_selects_highest_score(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
...
# 7
async def test_long_put_selects_lowest_score(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
...
# 8
async def test_risk_reward_ratio_positive(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    for p in output.proposals:
...
# 9
async def test_greeks_present_in_proposal(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    for p in output.proposals:
        assert "greeks" in p
...
# 10
async def test_composite_score_in_proposal(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    for p in output.proposals:
...
# 11
async def test_empty_candidates_raises_value_error(agent) -> None:
    with pytest.raises(ValueError):
        await agent.run(StrategyInput(candidates=[]))


# 12
async def test_long_call_strike_above_price(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    lc = next(p for p in output.proposals if p["strategy_type"] == StrategyType.LONG_CALL.value)
...
# 13
async def test_long_put_strike_below_price(agent, candidates) -> None:
    output = await agent.run(StrategyInput(candidates=candidates))
    lp = next(p for p in output.proposals if p["strategy_type"] == StrategyType.LONG_PUT.value)
...
# 14
async def test_run_date_used_for_expiry(agent, candidates) -> None:
    run_date = date(2025, 1, 1)
    output = await agent.run(StrategyInput(candidates=candidates, run_date=run_date))
    for p in output.proposals:
